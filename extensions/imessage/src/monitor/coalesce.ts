...
// mechanical extraction instead of a behavioral redesign. This is the generic
// same-sender debounce merge shared with Discord/Telegram/Slack; Apple's
// URL-preview split-sends are now coalesced upstream by imsg (>= 0.11.1).
 ...
 * downstream dispatch. Used when the general inbound debounce
 * (`messages.inbound`, off by default) flushes a bucket of more than one
 * same-sender text row into one agent turn — the same behavior other channels
 * (Discord, Telegram, Slack) apply.
 *
