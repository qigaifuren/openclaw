...
| `channels.bluebubbles.coalesceSameSenderDms`               | _(N/A — retired)_                         | Drop this key. `imsg` (>= 0.11.1) coalesces Apple URL-preview split-sends upstream, so no iMessage-side opt-in is needed; `openclaw doctor --fix` removes any stale `coalesceSameSenderDms`. See [iMessage docs § Split-send DMs](/channels/imessage#coalescing-split-send-dms-command--url-in-one-composition). |
...
| URL split-send coalescing                           | ✅                                  | ✅ (handled upstream by `imsg` >= 0.11.1; no config needed)                   |
